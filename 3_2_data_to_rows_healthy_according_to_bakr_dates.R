...
for (year in YEAR_DATA){
  ...
  scaled_healthy_cases <- bankrupt_cases*healthy_to_bankrupt_ratio
  #scaled_healthy_cases <- bankrupt_cases#*healthy_to_bankrupt_ratio # fixing empty rows
  
  healthy_year_cases_to_track <- scaled_healthy_cases[as.numeric(substring(names(scaled_healthy_cases),6,9)) == year]
  ...
  cat("\n")
  #scaled_healthy_cases[as.numeric(substring(names(scaled_healthy_cases),6,9)) == 2017]
  ...
  for(bID in myDataM0_IDs) {
    ...
    if (iterator >= 5000 & iterator %% 5000 == 0){
      ...
      if(scale_to_date == 1){
        write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_scaled_',year,'_part_',iterator,'.csv'), row.names = FALSE)
      } else {
        write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_NOT-scaled_',year,'_part_',iterator,'.csv'), row.names = FALSE)
  ...
  write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_scaled_',year,'_part_last.csv'), row.names = FALSE)
...
for (year in YEAR_DATA){
  fl <- list.files(paste0('data\\3_data_to_rows\\scaled\\'))
  ...
  for(i in 1:length(fl)){
    ...
    fileName <- fl[i]
    filePath <- paste0("data\\3_data_to_rows\\scaled\\",fileName)
...
write.csv(full_data_set, paste0('data\\3_data_to_rows\\FULL_HEALTHY.csv'), row.names = FALSE)

nrow(full_data_set)
nrow(full_bankrupt_data)

myData_full <- rbind(full_data_set,full_bankrupt_data)
nrow(myData_full)
length(unique(myData_full$ID))

set.seed(101)
rand <- sample(nrow(myData_full))
myData_full <- myData_full[rand,]
which(myData_full$STATUS == 1)

write.csv(myData_full, paste0('data\\3_data_to_rows\\FULL_DATA.csv'), row.names = FALSE)
myData_no2018 <- myData_full[fGetYearFromDate(myData_full$DATE) != 2018,]
write.csv(myData_no2018, paste0('data\\3_data_to_rows\\FULL_DATA_no2018.csv'), row.names = FALSE)
...
fl <- list.files(paste0('data\\3_data_to_rows\\2018_not_scaled\\'))
fullData <- read.csv("data\\3_data_to_rows\\FULL_DATA.csv", header=T, stringsAsFactors = FALSE, sep=',')
iterator <- 0
for(i in 1:length(fl)){
  
  fileName <- fl[i]
  filePath <- paste0("data\\3_data_to_rows\\2018_not_scaled\\",fileName)
...
nrow(full_data_set[full_data_set$STATUS==1,]) # The not scaled data set

nrow(fullData)-nrow(fullData[fullData$STATUS==1,])


full_data_set <- rbind(full_data_set,fullData)
nrow(full_data_set[full_data_set$STATUS==1,])
nrow(full_data_set)
full_data_set_only_2018 <- full_data_set[fGetYearFromDate(full_data_set$DATE) == 2018,]

nrow(full_data_set_only_2018)
nrow(full_data_set_only_2018[full_data_set_only_2018$STATUS==1,])/nrow(full_data_set_only_2018)

write.csv(full_data_set_only_2018, paste0('data\\3_data_to_rows\\FULL_DATA_ONLY_2018_ORIG_DISBALANCE_[TEST].csv'), row.names = FALSE)
