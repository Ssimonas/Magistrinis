...
data_part_M <- myData0_full[,which(colnames(myData0_full) %in% "pardavimai_pvm_dekl"):which(colnames(myData0_full) %in% "likutis")]
...
data_part_Y <- myData0_full[,col_id_start_Y1:col_id_end_Y1,col_id_start_Y2:col_id_end_Y2]

cor(myData0_full[,which(colnames(myData0_full) %in% "pardavimai_pvm_dekl"):which(colnames(myData0_full) %in% "likutis")], method = c("pearson", "kendall", "spearman"))
corrplot(data_part)
cor_M <- cor(data_part_M)
cor_Y <- cor(data_part_Y)
round(cor_M,2)
##### Pasalinami irasai
...
scale_to_date <- 0 # 0 - processes all companies fom this year (picks bankruptcy date by random), 1 - scales companies by bankruptcy dates
...
for (year in YEAR_DATA){
  ...
  #if (scale_to_date == 1){
  #  myDataM0_IDs <- sample(myDataM0_IDs, (year_bankrupcies+2000))
  #}
  ...
  for(bID in myDataM0_IDs) {
    ...
    if (iterator >= 5000 & iterator %% 5000 == 0){
      ...
      if(scale_to_date == 1){
        write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_scaled_',month_being_processed,'_ALL_',year,'_part_',iterator,'.csv'), row.names = FALSE)
      } else {
        write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_NOT-scaled_',month_being_processed,'_ALL_',year,'_part_',iterator,'.csv'), row.names = FALSE)
  ...
  write.csv(myDataRows, paste0('data\\3_data_to_rows\\HEALTHY_scaled_',month_being_processed,'_ALL_',year,'_part_last.csv'), row.names = FALSE)
...
for (year in YEAR_DATA){
  fl <- list.files(paste0('data\\3_data_to_rows\\JANUARY\\'))
  ...
  for(i in 1:length(fl)){
    ...
    fileName <- fl[i]
    filePath <- paste0("data\\3_data_to_rows\\JANUARY\\",fileName)
  ...
  # if (nrow(full_data_set) != nrow(unique(full_data_set))){
  #   full_data_set <- unique(full_data_set)
  # }
  # length(unique(full_data_set$ID))
}
write.csv(full_data_set, paste0('data\\3_data_to_rows\\JANUARY\\FULL_HEALTHY_JAN.csv'), row.names = FALSE)

full_bankrupt_data_MONTH <- full_bankrupt_data[substring(full_bankrupt_data$DATE,3,5) == "JAN",]

names(full_bankrupt_data_MONTH)
names(full_data_set)

full_bankrupt_data_MONTH <- full_bankrupt_data_MONTH[,!names(full_bankrupt_data_MONTH) %in% c("VADOV_POK_M_1", "VADOV_POK_M_2",
                                                                                              "VADOV_POK_M_3", "VADOV_POK_M_4",
                                                                                              "VADOV_POK_M_5", "VADOV_POK_M_6",
                                                                                              "VADOV_POK_M_7", "VADOV_POK_M_8",
                                                                                              "VADOV_POK_M_9", "VADOV_POK_M_10",
                                                                                              "VADOV_POK_M_11", "VADOV_POK_M_12",
                                                                                              "laik_tipas_pvm_M_Y_1", "laik_tipas_pvm_P_Y_1")]

nrow(full_data_set)
nrow(full_bankrupt_data_MONTH)

myData_full <- rbind(full_data_set,full_bankrupt_data_MONTH)
set.seed(101)
rand <- sample(nrow(myData_full))
myData_full <- myData_full[rand,]
myData_no2018 <- myData_full[fGetYearFromDate(myData_full$DATE) != 2018,]
write.csv(myData_no2018, paste0('data\\3_data_to_rows\\MONTHLY\\JANUARY\\FULL_DATA_JAN_no2018.csv'), row.names = FALSE)

myData_only2018 <- myData_full[fGetYearFromDate(myData_full$DATE) == 2018,]
write.csv(myData_only2018, paste0('data\\3_data_to_rows\\MONTHLY\\JANUARY\\FULL_DATA_JAN_only2018.csv'), row.names = FALSE)

nrow(myData_full[myData_full$STATUS==1,])/nrow(myData_full)



full_bankrupt_data <- read.csv(paste0('data\\3_data_to_rows\\FULL_BANKR.csv'),header=T, stringsAsFactors = FALSE, sep=',')

full_bankrupt_data <- full_bankrupt_data[,!names(full_bankrupt_data) %in% c("VADOV_POK_M_1", "VADOV_POK_M_2",
                                                                                              "VADOV_POK_M_3", "VADOV_POK_M_4",
                                                                                              "VADOV_POK_M_5", "VADOV_POK_M_6",
                                                                                              "VADOV_POK_M_7", "VADOV_POK_M_8",
                                                                                              "VADOV_POK_M_9", "VADOV_POK_M_10",
...
for(month_num in 1:12){
  ...
  month_string <- month.abb[month_num]
  
  files_path <- paste0('data\\3_data_to_rows\\MONTHLY\\',month_num,'\\parts\\')
  
  fl <- list.files(files_path)
  ...
  for(i in 1:length(fl)){
    ...
    fileName <- fl[i]
    filePath <- paste0(files_path,fileName)
  ...
  month_bankrupt_data <- full_bankrupt_data[substring(full_bankrupt_data$DATE,3,5) == month_string,]
  month_bankrupt_IDs <- unique(month_bankrupt_data$ID)
  
  cat(month_string,"\n")
  cat(length(unique(full_data_set$ID)),"\n")
  full_data_set <- full_data_set[!full_data_set$ID %in% month_bankrupt_IDs,]
  cat(length(unique(full_data_set$ID)),"\n")
  
  full_data_set <- rbind(full_data_set,month_bankrupt_data)
  ...
  write.csv(full_data_set_no2018, paste0('data\\3_data_to_rows\\MONTHLY\\',month_num,'\\train_',month_num,'_no2018.csv'), row.names = FALSE)
  
  full_data_set_only2018 <- full_data_set[fGetYearFromDate(full_data_set$DATE) == 2018,]
  write.csv(full_data_set_only2018, paste0('data\\3_data_to_rows\\MONTHLY\\',month_num,'\\test_',month_num,'_only2018.csv'), row.names = FALSE)
  
  rm(full_data_set_no2018,full_data_set_only2018,full_data_set,myData)
}
